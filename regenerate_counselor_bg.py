...
t = {
    "id": "counselor",
    "color": "dark forest green", # Switched from emerald green to dark forest green for high contrast text readability
    "motif": "classical counseling room interior bookshelves and elegant window frames"
}
...
def main():
    print("Regenerating Counselor background with a darker green tone for better text contrast...")
    
    prompt = (
        f"A highly detailed, intricate hand-drawn G-pen style engraving background. "
        f"Colored entirely in a monochromatic color palette of {t['color']} (with only very subtle shifts in value: slightly darker {t['color']} shadows and slightly lighter {t['color']} highlights). "
    ...
    for try_idx in range(3):
        try:
            ...
            if response.status_code == 200 and res_json.get("code") == 200:
                ...
                print(f"Submitted counselor_bg: Task ID = {task_id}")
                break
            else:
    ...
    print("\nPolling counselor background task status...")
    ...
    while True:
        ...
        try:
            ...
            if r.status_code == 200 and res_json.get("code") == 200:
                ...
                print(f"Task counselor_bg retry status: {status}")
                ...
                if status == "success":
                    ...
                    if url:
                        out_path = os.path.join(BG_DIR, "counselor_bg.png")
                        print("  Downloading counselor background...")
                        img_data = requests.get(url, timeout=30).content
                        with open(out_path, 'wb') as f:
                            f.write(img_data)
                        print("  Successfully finished counselor background.")
                        break
                    else:
                        print("  No URL found.")
                        break
