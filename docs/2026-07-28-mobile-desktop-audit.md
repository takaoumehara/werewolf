...
### I-1【P0】オープニング動画4.16MBが、見ない人にも無条件で降ってくる
...
### I-3【P1】Google Fonts が1.27MB、うち1ファミリーは完全に未使用
...
### I-4【P2】117MBの画像が1時間しかキャッシュされない
...
| 4 | **B（オーバーレイ共通基盤）** | 共通ヘルパー1つで10件が同時に解消。デスクトップ対応もここで片付く。**次はここから** |
...
| 章 | 内容 |
|---|---|
| **B** | オーバーレイの共通基盤（スクリム・背景スクロールロック・フォーカストラップ・ARIA）。10件が1つの原因に由来する |
| **G** | アクセシビリティ（画面遷移のフォーカス移動、対象選択のキーボード操作、`aria-live`、ラベル） |
| **I** | アセット最適化（動画4.16MBの `preload="none"`、サムネの縮小、フォント、キャッシュヘッダ） |
| **F** | root `index.html` と `mobile_app.html` の分岐解消（方針決定が先） |
| **H** | 接続監視（`onDisconnect`）、長押しの `-webkit-touch-callout`、ボタンの hover |
