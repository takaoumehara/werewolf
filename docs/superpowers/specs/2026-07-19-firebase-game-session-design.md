...
roomCreateRateLimits/{uid}
  windowStartedAt
  count
```

`authoritative`、完全イベント、処理済みコマンド、`joinState`、`pairingCodes`、
`roomCreateRateLimits`はAdmin SDKだけが
...
- pairing codeは候補のreadだけで確保せず、code pathのtransactionで原子的に予約する。
...
- `roomCreateRateLimits` はクライアントread/write禁止。
- `meta` と `players` はroom memberだけがread可能。
- `players/{uid}/name`, `connected`, `lastSeenAt` だけ既存room memberの本人がwrite可能。型、長さ、非負時刻、削除禁止をvalidateする。
- `game/public` と `game/publicEvents` はroom memberだけがread可能、write禁止。
- `game/privateViews/{uid}` はroom member本人だけがread可能、write禁止。
- `game/authoritative`, `game/events`, `game/processedCommands` はクライアントread/write禁止。
- room親パスへread/writeを与えない。
- hostにもmeta/statusやgame stateの直接writeは許可せず、ゲーム操作はCallable Functionsだけを通す。
