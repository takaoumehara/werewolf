...
t = {
    "id": "knights",
    "color": "deep royal blue", # Switched from sky blue to deep royal blue for better white text readability
    "motif": "medieval castle ramparts, fortress stone walls, and cross heraldry patterns"
}
...
def main():
    print("Regenerating Knights background with a darker blue tone for better text contrast...")
    ...
    for try_idx in range(3):
        try:
            ...
            if response.status_code == 200 and res_json.get("code") == 200:
                ...
                print(f"Submitted knights_bg: Task ID = {task_id}")
                break
            else:
    ...
    print("\nPolling knights background task status...")
    ...
    while True:
        ...
        try:
            ...
            if r.status_code == 200 and res_json.get("code") == 200:
                ...
                print(f"Task knights_bg retry status: {status}")
                ...
                if status == "success":
                    ...
                    if url:
                        out_path = os.path.join(BG_DIR, "knights_bg.png")
                        print("  Downloading knights background...")
                        img_data = requests.get(url, timeout=30).content
                        with open(out_path, 'wb') as f:
                            f.write(img_data)
                        print("  Successfully finished knights background.")
                        break
                    else:
                        print("  No URL found.")
                        break
