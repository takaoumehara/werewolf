...
t = {"id": "spy", "color": "dark charcoal black", "motif": "dark industrial factory steel girders, pipes, and metal mesh grids"}
...
def main():
    print("Retrying failed spy background task with darker tone...")
    ...
    for try_idx in range(3):
        try:
            ...
            if response.status_code == 200 and res_json.get("code") == 200:
                ...
                print(f"Submitted spy_bg: Task ID = {task_id}")
                break
            else:
    ...
    print("\nPolling spy background task status...")
    ...
    while True:
        ...
        try:
            ...
            if r.status_code == 200 and res_json.get("code") == 200:
                ...
                print(f"Task spy_bg retry status: {status}")
                ...
                if status == "success":
                    ...
                    if url:
                        out_path = os.path.join(BG_DIR, "spy_bg.png")
                        print("  Downloading spy background...")
                        img_data = requests.get(url, timeout=30).content
                        with open(out_path, 'wb') as f:
                            f.write(img_data)
                        print("  Successfully finished spy background.")
                        break
                    else:
                        print("  No URL found.")
                        break
