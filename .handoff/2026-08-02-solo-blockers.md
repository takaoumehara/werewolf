...
作業ブランチ: `claude/werewolf-solo-ai-fix-zc4emn`（**draft PR #1**）。コードは push 済み。

| # | 前回の宿題 | 結論 |
|---|---|---|
| ① | AIが喋らない（鍵未設定） | 鍵が無くても**ローカル発話で卓が最後まで回る**。簡易モードは画面に出す |
| ② | ルーム番号 TTL 5分 | **二択を捨てた**。合言葉（新規参加・12h・卓が waiting の間だけ）と復帰（`resumeRoom`・uid・寿命無し）を別の道に分けた |
...
`mobile_app` · `design_system` pass。

UI回帰の新2項目は**壊して落ちることを確認済み**（`#resumeCard` 改名 +
`requestAiReplyWave` 除去で 2件 FAIL）。
...
| File | Status | Open questions |
|---|---|---|
| **2026-08-02-solo-blockers.md** | **今回の正本** | none |
| 2026-08-02-playability-ux.md | 前回の正本（§6/§7 に解決済の追記）| none |
| deploy.md | agreed。§2 に**鍵の切り分け手順**を追加 | 実鍵の登録はユーザーの端末 |
| promo-video-plan.md | 台本として完成 | ユーザーがブラウザで実行する |
| superforge.md | 言語設定・pinned | 会話も docs も日本語 |
...
- 復帰は**タップ1回**要る（タイトル → 部屋選び →「前の卓に戻る」）。
  自動復帰にしなかったのは、新しく遊びたい人を引き戻す事故の方が重いから
- プライベート閲覧では復帰の控えが残らない（遊べるが戻れない）
...
6. `mobile_app.html` — 単一ファイルの本体。`public/index.html` は
   `bash tools/build-public.sh` の**生成物**（直接編集しない）

## 決めたこと（今回追加分）

- **合言葉と復帰は別のもの**。合言葉は「まだ入っていない人」の鍵、復帰は
  「既に席がある人」を戻すもの。**新規参加の締切は時計ではなく卓の状態**
  （`meta.status === 'waiting'` の間だけ）。TTL 12h は合言葉空間の占有上限にすぎない
- **既に一員なら、寿命も卓の状態も問わず合言葉を通す**（復帰導線を知らない人の保険）
- **「鍵が無い」は失敗ではなく設定状態**。`errors` に数えず、`speechMode:"local"` で表す
- **埋め合わせても、失敗した事実は消さない**。ローカル発話は `source:"local"` を
  chat に残し、生成の失敗は引き続き `errors` と `console.error` に出す
- **発話は波に分ける**。1波目=全員の第一声、2波目以降=人間への返し。上限3波は
  **サーバー側で強制**する（クライアントの実装ミスで課金が伸びないように）
